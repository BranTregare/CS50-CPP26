...
{
  SECTION("VISA 16-digit") { REQUIRE(test_validate(4111111111111111) == CardType::VISA); }

  SECTION("AMEX") { REQUIRE(test_validate(378282246310005) == CardType::AMEX); }

  SECTION("MASTERCARD") { REQUIRE(test_validate(5105105105105100) == CardType::MASTERCARD); }

  SECTION("VISA 13-digit") { REQUIRE(test_validate(4222222222222) == CardType::VISA); }
}
...
TEST_CASE("Invalid cards fail due to length or unsupported prefix", "[invalid][prefix]")
{
  ...
  // With -Werror and constexpr validation, this is caught at compile time.
  // Uncommenting this test may cause compilation to fail.
  // REQUIRE(test_validate(41111111111111112222ULL) == CardType::INVALID); // Too long
}